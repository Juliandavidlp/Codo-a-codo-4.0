...
Esta colaboración entre clases se manifiesta como la posibilidad de utilizar en una clase objetos que son instancias de una clase diferente. Este mecanismo potencia el alcance que tiene 
la Programación Orientada a Objetos, facilitando aún más el desarrollo de soluciones a problemas complejos sin necesidad de escribir código extenso o de difícil de lectura.

Para entender cómo se produce la colaboración entre clases voy a desarrollar un ejemplo, explicando paso a paso lo que sucede:
...
Para cada entidad necesito crear una clase e instanciar la cantidad necesaria de objetos de la misma. Una vez identifique las clases, que voy a llamar "Cliente" y "Banco" de manera respectiva, 
...
"""
class Banco:
    def __init__(self): #Instancio tres objetos de la clase cliente y los asigno a la clase banco para operar con los mismos.
    ...
    # cliente2 para extraer 1000 pesos de su cuenta.
...
se llama al método retornar_monto() e imprimir() de la clase Cliente. Entonces ambas clases colaboran para resolver el problema planteado.
"""
#Programa principal:
Banco_Nación=Banco()
Banco_Nación.operar()
...
Por lo tanto, cuando vuelvo a consultar su valor, Python imprime "21" que es el valor de su atributo de instancia.

Si existe un atributo de clase y uno de instancia con el mismo nombre, tiene prioridad el de la instancia. El cambio de valor no afecta a dato2, que sigue mostrando el valor del atributo de clase.
"""